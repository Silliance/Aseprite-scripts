...
-- added by Silliance
function matrix.sum(m, dim)
	if matrix.rows(m) == 1 then
		return (m * matrix:new(matrix.columns(m), 1, 1))[1][1]
	elseif matrix.columns(m) == 1 then
		return (matrix:new(1, matrix.rows(m), 1) * m)[1][1]
	else
		if dim == 1 then
			return m * matrix:new(matrix.columns(m), 1, 1)
		elseif dim == 2 then
			return matrix:new(1, matrix.rows(m), 1) * m
		else
			return matrix:new(1, matrix.rows(m), 1) * m * matrix:new(matrix.columns(m), 1, 1)
		end
	end
end

function matrix.dotpow(m, x)
	for i = 1, matrix.rows(m), 1 do
		for j = 1, matrix.columns(m), 1 do
			m[i][j] = m[i][j] ^ x
		end
	end
	return m
